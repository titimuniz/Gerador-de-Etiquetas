...
export const SebraePaper: React.FC<SebraePaperProps> = ({ logo, items, fontSettings, sebraeConfig }) => {
  ...
  // Modelo SEBRAE:
  // 4 fileiras horizontais por página.
  ...
  /**
   * DIMENSÕES REFINADAS (A4: 210x297mm)
   * Altura Total Etiqueta: 65mm
   * Borda Externa (Topo/Fundo): 3mm cada
   * Faixa Central de Dobra: 4mm
   * Faces Brancas (Frente/Verso): ~27.5mm cada
   */
  const BORDER_THICKNESS = '3mm';
  const TAG_TOTAL_HEIGHT = '65mm'; 
  const FOLD_GAP_HEIGHT = '4mm'; 
  const FACE_HEIGHT = '27.5mm'; 
  const VERTICAL_ROW_GAP = '6mm'; 
                    ...
                    {Array.from({ length: tagsPerRow }).map((_, colIdx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {/* Na face invertida, para o Logo ficar NO TOPO (perto da borda externa), 
                                ele deve estar na base do container no código antes da rotação */}
                            
                            {/* 1. O Nome do Item (ficará mais próximo à dobra após rotação) */}
                            <h2 
                            ...
                            {/* 2. O Logo (ficará no topo da etiqueta após rotação) */}
                          ...
                          {/* Faixa de Dobra Central (Vincado) */}
                          <div 
                            style={{ 
                          ...
                          >
                            {/* Na face normal, o logo fica no topo (perto da dobra) */}
